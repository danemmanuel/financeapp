...
export class NumericInputComponent
  ...
  private init(options) {
    ...
    if (typeof formatFn === 'string') {
      formatFn = ((rformat) => (val) => rformat.test(val))(
  ...
  moveCursor() {
    ...
    const elCursor = this.ks.inputElement.querySelector(
    ...
    elCursor.style.transform = `translateX(${Math.min(
      maxVisibleWidth - 1,
      cursorOffset
    )}px)`;
    elText.style.transform = `translateX(${Math.min(
      0,
      maxVisibleWidth - cursorOffset
    )}px)`;
  }
  ...
  openKeyboard() {
    ...
    animate(
      (timestamp, frame, frames) => {
        ...
        elKeyboard.style.transform = `translateY(${
          ((frames - frame) / frames) * 100
        }%)`;
      },
      () => {},
      10
